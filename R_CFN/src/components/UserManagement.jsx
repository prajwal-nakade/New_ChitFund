import { BookUser, Search, SquarePen, Trash } from 'lucide-react'
...
const UserManagement = ({ data, setUserEntriesData }) => {
  ...
  return (
    <div className="bg-white border rounded p-4 mt-10 border-neutral-300 shadow-lg">
                  ...
                  <td className="border border-neutral-300 p-2">
                    <div className='flex items-center justify-center gap-2'>
                      <button onClick={() => {
                        ...
                        setActiveModal('edit')
                      }} className='p-1 bg-yellow-500 rounded-sm hover:bg-yellow-600 transition-colors duration-300 cursor-pointer relative group'><SquarePen size={16} />
                      ...
                      <button onClick={() => handleDelete(item.id, item.firstname)} className='relative group  p-1 bg-red-500 text-white rounded-sm hover:bg-red-600 transition-colors duration-300 cursor-pointer'><Trash size={16} />
                      ...
                      <button onClick={() => {
                        setSelectedUser(item)
                        setActiveModal('view')
                      }} className='relative group p-1 bg-slate-400 text-white rounded-sm hover:bg-slate-500 transition-colors duration-300 cursor-pointer'><BookUser size={16} />
      ...
      {activeModal === "view" && (<ViewUserDetails user={selectedUser} onClose={() => setSelectedUser(null)} />)}
    </div>
  )
}
