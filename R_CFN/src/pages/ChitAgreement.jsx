...
const ChitAgreement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        scheduled_last_date_of_payment:
          formData.scheduled_last_date_of_payment || null,
      ...
      if (response.success) {
        toast.success(
          `Chit Agreement is Created for Customer ${displayData.fullName} having Agreement ID : ${response.id}`,
        );
        setTimeout(() => {
  ...
  const autoFill = (chit) => {
    ...
    const installmentAmt =
      chitDuration > 0 ? Math.round((chitAmt / chitDuration) * 100) / 100 : "";
          ...
          <div className="flex flex-col lg:flex-row w-full px-5 pb-2 border-neutral-200 gap-4">
            <div className="flex flex-col items-start w-full text-sm  ">
              ...
              <input
                ...
                type="text"
                placeholder=""
                className="border border-neutral-300 w-133 rounded-md px-3 py-1"
              />
            </div>
          </div>
